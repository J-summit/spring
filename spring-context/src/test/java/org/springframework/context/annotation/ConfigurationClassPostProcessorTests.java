...
public class ConfigurationClassPostProcessorTests {
	...
	public void testEnhancementIsPresentBecauseSingletonSemanticsAreRespected() {
		...
		beanFactory.registerBeanDefinition("springconfig", new RootBeanDefinition(SingletonBeanConfig.class));
	...
	public void configurationIntrospectionOfInnerClassesWorksWithDotNameSyntax() {
		DefaultListableBeanFactory beanFactory = new DefaultListableBeanFactory();
		beanFactory.registerBeanDefinition("springconfig", new RootBeanDefinition(getClass().getName() + ".SingletonBeanConfig"));
		ConfigurationClassPostProcessor pp = new ConfigurationClassPostProcessor();
		pp.postProcessBeanFactory(beanFactory);
		Foo foo = beanFactory.getBean("foo", Foo.class);
		Bar bar = beanFactory.getBean("bar", Bar.class);
