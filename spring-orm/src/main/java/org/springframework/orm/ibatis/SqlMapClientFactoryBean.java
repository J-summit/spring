...
public class SqlMapClientFactoryBean implements FactoryBean<SqlMapClient>, InitializingBean {
	...
	/**
	 * Set the location of the iBATIS SqlMapClient springconfig file.
	 * A typical value is "WEB-INF/sql-map-springconfig.xml".
	...
	/**
	 * Set multiple locations of iBATIS SqlMapClient springconfig files that
	 ...
	 * in a sql-map-client springconfig file. This property being based on Spring's
	 ...
	 * alternative to a {@code &lt;properties&gt;} tag in the sql-map-springconfig.xml
	 * file. Will be used to resolve placeholders in the springconfig file.
	 * @see #setConfigLocation
	 ...
	 * Will be exposed at springconfig time for TypeHandler implementations.
	 ...
	 * @param configLocations the springconfig files to load from
	 * @param properties the SqlMapClient properties (if any)
	 * @return the SqlMapClient instance (never {@code null})
	 * @throws IOException if loading the springconfig file failed
	...
	protected SqlMapClient buildSqlMapClient(
		...
		for (Resource configLocation : configLocations) {
			...
			try {
				...
				throw new NestedIOException("Failed to parse springconfig resource: " + configLocation, ex.getCause());
			}
		}
